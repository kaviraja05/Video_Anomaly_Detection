  ...
  return (
    <div className="w-full max-w-5xl mx-auto">
      <div className="flex items-center gap-3 mb-8">
        ...
        <div>
          <h2 className="text-2xl font-bold text-slate-200">Analysis History</h2>
          <p className="text-slate-500 text-sm">View your previously uploaded video results</p>
        </div>
      </div>
      
      {loading ? (
        <div className="text-center py-12 text-slate-400 animate-pulse">Loading history...</div>
      ) : (
        <div className="bg-slate-900/40 rounded-xl border border-slate-800/80 overflow-hidden shadow-xl backdrop-blur-sm">
          {history.length > 0 ? (
                    ...
                    <tr key={idx} className="hover:bg-slate-800/30 transition-colors">
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="text-slate-200 font-medium">{record.video_name}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
            ...
            <div className="py-16 text-center">
              <div className="inline-flex justify-center items-center w-16 h-16 rounded-full bg-slate-800/50 text-slate-500 mb-4">
                <Activity size={32} />
              </div>
              <h3 className="text-lg font-medium text-slate-300 mb-1">No analysis history</h3>
              <p className="text-slate-500 max-w-sm mx-auto">Upload and analyze your first video to see your history logs appear here.</p>
            </div>
          )}
        </div>
      )}
